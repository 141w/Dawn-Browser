...
const { tabGroups, tabSidebarOpen, isAiGrouping, createGroup, renameGroup, deleteGroup, addTabToGroup, toggleSidebar } = useTabGroups()
  ...
  <div class="ts-root" :class="{ collapsed: !tabSidebarOpen }">
    <div class="ts-header">
      <button class="ts-toggle" @click="toggleSidebar" :title="tabSidebarOpen ? 'Collapse' : 'Expand'">
        <svg v-if="tabSidebarOpen" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 17l-5-5 5-5"/><path d="M18 17l-5-5 5-5"/></svg>
        <svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 17l5-5-5-5"/><path d="M6 17l5-5-5-5"/></svg>
      </button>
      <span v-if="tabSidebarOpen" class="ts-title">Tabs</span>
      <button v-if="tabSidebarOpen" class="ts-icon-btn" @click="showNewGroupInput = !showNewGroupInput" title="New Group">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg>
      </button>
      <button v-if="tabSidebarOpen" class="ts-group-btn" @click="$emit('auto-group')" :disabled="isAiGrouping" :title="isAiGrouping ? 'AI analyzing...' : 'AI Smart Group'">
    ...
    <div v-if="tabSidebarOpen" class="ts-body">
      ...
      <div v-if="bookmarks.length > 0" class="ts-bookmarks">
        ...
        <div v-for="bm in bookmarks" :key="bm.id" class="ts-tab" @click="selectTab(bm.url)">
...
<style scoped>
.ts-root { width: 240px; flex-shrink: 0; display: flex; flex-direction: column; background: rgba(28,28,28,0.02); border-right: 1px solid #eceae4; transition: width 0.15s; overflow: hidden; }
.ts-root.collapsed { width: 40px; }
.ts-header { display: flex; align-items: center; gap: 4px; padding: 6px 4px; border-bottom: 1px solid #eceae4; flex-shrink: 0; min-height: 36px; }
.ts-toggle { display: flex; align-items: center; justify-content: center; width: 24px; height: 24px; background: transparent; border: none; border-radius: 4px; color: #8a8a88; cursor: pointer; flex-shrink: 0; }
.ts-toggle:hover { background: rgba(28,28,28,0.06); color: #1c1c1c; }
.ts-title { font-size: 12px; font-weight: 600; color: #1c1c1c; flex: 1; }
.ts-icon-btn { display: flex; align-items: center; justify-content: center; width: 22px; height: 22px; background: transparent; border: none; border-radius: 4px; color: #8a8a88; cursor: pointer; flex-shrink: 0; }
.ts-icon-btn:hover { background: rgba(28,28,28,0.06); color: #1c1c1c; }
.ts-group-btn { display: flex; align-items: center; justify-content: center; width: 22px; height: 22px; background: transparent; border: none; border-radius: 4px; color: #8a8a88; cursor: pointer; flex-shrink: 0; }
...
.ts-input { flex: 1; padding: 3px 6px; background: #fcfbf8; border: 1px solid #eceae4; border-radius: 4px; font-size: 11px; font-family: inherit; color: #1c1c1c; outline: none; }
.ts-input:focus { border-color: rgba(28,28,28,0.3); }
.ts-input-btn { padding: 3px 8px; background: rgba(28,28,28,0.06); border: 1px solid #eceae4; border-radius: 4px; font-size: 10px; font-family: inherit; color: #1c1c1c; cursor: pointer; }
.ts-input-btn:disabled { opacity: 0.3; }

.ts-group { margin-bottom: 2px; position: relative; }
.ts-group-header { display: flex; align-items: center; gap: 4px; padding: 4px 6px; border-radius: 4px; cursor: pointer; transition: all 0.1s; font-size: 11px; font-weight: 600; color: #5f5f5d; }
...
.ts-rename-input { flex: 1; padding: 1px 4px; background: #fcfbf8; border: 1px solid #2563eb; border-radius: 3px; font-size: 11px; font-weight: 600; font-family: inherit; color: #1c1c1c; outline: none; min-width: 0; }
.ts-group-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.ts-group-count { font-size: 10px; color: #8a8a88; background: rgba(28,28,28,0.06); padding: 0 5px; border-radius: 8px; flex-shrink: 0; }

.ts-ctx-menu { position: absolute; top: 100%; left: 8px; z-index: 50; background: #fcfbf8; border: 1px solid #eceae4; border-radius: 6px; box-shadow: rgba(0,0,0,0.1) 0 4px 16px; padding: 2px; min-width: 100px; }
.ts-ctx-menu button { display: block; width: 100%; padding: 5px 10px; background: transparent; border: none; border-radius: 4px; font-size: 11px; font-family: inherit; color: #1c1c1c; cursor: pointer; text-align: left; }
.ts-ctx-menu button:hover { background: rgba(28,28,28,0.06); }

.ts-group-tabs { padding-left: 12px; }
.ts-empty-group { font-size: 10px; color: #8a8a88; padding: 4px 8px; font-style: italic; }

.ts-bookmarks { margin-bottom: 8px; }
.ts-bookmarks-label { font-size: 9px; font-weight: 700; color: #8a8a88; text-transform: uppercase; letter-spacing: 0.8px; padding: 4px 8px 2px; }

.ts-ungrouped { }
.ts-tab { display: flex; align-items: center; gap: 6px; padding: 6px 8px; border-radius: 4px; cursor: pointer; transition: all 0.1s; font-size: 12px; color: #5f5f5d; position: relative; }
...
.ts-tab-domain { font-size: 9px; color: #8a8a88; max-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex-shrink: 0; }
.ts-tab-close { display: flex; align-items: center; justify-content: center; width: 16px; height: 16px; background: transparent; border: none; border-radius: 3px; color: #8a8a88; cursor: pointer; font-size: 12px; opacity: 0; }
