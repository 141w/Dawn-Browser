...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  windowMinimize: () => ipcRenderer.invoke('window-minimize'),
    ...
    agentMemoryAddMemory: (content, tags, sourceConvId) => ipcRenderer.invoke('agent-memory:add-memory', content, tags, sourceConvId),
    agentMemoryListMemories: (limit) => ipcRenderer.invoke('agent-memory:list-memories', limit),
    agentMemoryDeleteMemory: (id) => ipcRenderer.invoke('agent-memory:delete-memory', id),
    agentMemoryClearMemories: () => ipcRenderer.invoke('agent-memory:clear-memories'),
    agentMemorySearchMemories: (keywords, limit) => ipcRenderer.invoke('agent-memory:search-memories', keywords, limit),
    agentMemoryExtractKeywords: (text) => ipcRenderer.invoke('agent-memory:extract-keywords', text),
    agentMemoryRecordRecall: (memoryId, query, queryHash) => ipcRenderer.invoke('agent-memory:record-recall', memoryId, query, queryHash),
    agentMemoryGetRecallStats: (memoryId) => ipcRenderer.invoke('agent-memory:get-recall-stats', memoryId),
    agentMemoryListMemoriesByType: (type, limit) => ipcRenderer.invoke('agent-memory:list-memories-by-type', type, limit),
    agentMemoryPromoteMemories: (cfg) => ipcRenderer.invoke('agent-memory:promote-memories', cfg),
    agentMemorySaveDailyNote: (dateStr, title, entries) => ipcRenderer.invoke('agent-memory:save-daily-note', dateStr, title, entries),
    agentMemoryGetDailyNotes: (days) => ipcRenderer.invoke('agent-memory:get-daily-notes', days),
    agentMemoryReadDailyNote: (dateStr) => ipcRenderer.invoke('agent-memory:read-daily-note', dateStr),
    pluginListDirs: () => ipcRenderer.invoke('plugin:list-dirs'),
    pluginReadManifest: (name) => ipcRenderer.invoke('plugin:read-manifest', name),
    pluginExecuteTool: (pluginName, toolName, args) => ipcRenderer.invoke('plugin:execute-tool', pluginName, toolName, args),
    pluginExecuteHook: (pluginName, hookName, data) => ipcRenderer.invoke('plugin:execute-hook', pluginName, hookName, data),
    pluginInstallFromPath: (sourceDir, name) => ipcRenderer.invoke('plugin:install-from-path', sourceDir, name),
    pluginUninstall: (name) => ipcRenderer.invoke('plugin:uninstall', name),
    pluginGetAllTools: () => ipcRenderer.invoke('plugin:get-all-tools'),
