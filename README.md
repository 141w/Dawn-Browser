...
- **右键上下文菜单**：上下文感知动态菜单 — 选中文字时显示搜索/AI翻译/AI解释/AI总结，链接上显示新标签打开/复制链接，图片上显示保存/复制地址，空白处显示源代码/打印/新窗口打开
...
### 技能系统
- 从 GitHub 仓库或 ZIP 文件一键安装技能
- 内置 3 个技能：浏览器自动化、Markdown 编辑、技能创建
- 技能安装/卸载/启用/禁用/排序
- 技能 Prompt 自动注入 AI 上下文
- YAML 多行描述（> / |）正确解析
...
- 标签侧栏可折叠
- AI 侧栏宽度可拖拽
- **深色/浅色主题切换**：CSS 变量系统，全组件适配，设置页一键切换
...
- 快捷键速查表
- 界面语言（中文 / English）
- **右键菜单 AI 功能自定义**：添加/删除/排序/启用禁用自定义 AI 动作，支持 Prompt 模板（{text} 占位符）
...
| 模块 | 文件数 | 代码行数 |
|------|--------|----------|
| Electron 主进程 | 10 | ~2,500 |
| Vue 组件 | 10 | ~3,500 |
| Composables | 18 | ~4,000 |
| **合计** | **38** | **~10,000** |
...
### 上下文菜单增强（最新）
- **智能右键菜单**：根据右键位置（文字/链接/图片/空白）动态显示不同菜单项
- **AI 集成**：选中文字后右键可直接 AI 翻译/解释/总结，自动打开侧边栏发送
- **自定义 AI 动作**：设置页可添加自定义 Prompt 模板，支持排序和启用/禁用
- **实时同步**：设置修改后右键菜单立即更新
...
### 技能系统
- YAML 多行描述解析修复（description: > 格式）
- 技能执行时自动注入 SKILL.md 全文到 AI Prompt
- 从 GitHub/ZIP 安装技能
